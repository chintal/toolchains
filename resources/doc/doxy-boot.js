$( document ).ready(function() {
  ...
  */
  ...
  function rebuilddlcontent(dlelem){
      n = dlelem.children("dd").length;
      if (n <= 1){
          return("<p>" + dlelem.children("dd").first().html() + "</p>");
      }
      rval = "<ul>";
      dlelem.children("dd").each(function(){
          rval += ("<li>" + $(this).html() + "</li>");
      });
      rval += "</ul>";
      return(rval);
  };
  /* notes */
  $('.contents > .textblock > dl.note').each(function(){
      $(this).after('<div class="bs-callout bs-callout-info">' 
            + "<h4>" + $(this).children("dt").html() + "</h4>"
            + rebuilddlcontent($(this)) + "</div>");
  });
  
  /* warnings */
  $('.contents > .textblock > dl.warning').each(function(){
      $(this).after('<div class="bs-callout bs-callout-warning">' 
            + "<h4>" + $(this).children("dt").html() + "</h4>"
            + rebuilddlcontent($(this)) + "</div>");
  });
  
  /* seealso */
  $('.contents > .textblock > dl.see').each(function(){
      $(this).after('<div class="bs-callout bs-callout-primary">' 
            + "<h4>" + $(this).children("dt").html() + "</h4>"
            + rebuilddlcontent($(this)) + "</div>");
  });
